...
int main(int argc, char **argv)
{
	...
	if( argc < 6 )
 	...
 	ifstream in_file(argv[1], ios::binary);
   	...
   	std::string str4(argv[6]); //
   	
	ifstream is4;
 	is4.open (argv[6], ios::in | ios::binary);

 	INT option;
 	std::stringstream(str4)>>option;
	...
	else
	{
		...
		unsigned char * text = ( unsigned char * ) malloc (  ( text_size + 1 ) * sizeof ( unsigned char ) );
	
		ifstream inp;
	 	inp.open (argv[1], ios::in | ios::binary);
		...
		bd_anchors( text, argv[1], ell, k, text_anchors, option );
